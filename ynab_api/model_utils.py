...
class OpenApiModel(object):
    ...
    def set_attribute(self, name, value):
        ...
        if get_simple_class(name) != str:
            error_msg = type_error_message(var_name=name,
                                           var_value=name,
                                           valid_classes=(str, ),
                                           key_type=True)
            raise ApiTypeError(error_msg,
        ...
        if (name, ) in self.validations:
            check_validations(self.validations, (name, ), value,
                              self._configuration)
    ...
    def __new__(cls, *args, **kwargs):
        ...
        if len(args) == 1:
            ...
            if issubclass(cls,
                          ModelComposed) and allows_single_value_input(cls):
                model_kwargs = {}
                oneof_instance = get_oneof_instance(cls,
                                                    model_kwargs,
                                                    kwargs,
                                                    model_arg=arg)
        ...
        new_cls = get_discriminator_class(cls, discr_propertyname_py,
                                          discr_value, [])
        if new_cls is None:
            path_to_item = kwargs.get('_path_to_item', ())
            disc_prop_value = kwargs.get(discr_propertyname_js,
                                         kwargs.get(discr_propertyname_py))
            raise ApiValueError(
        ...
        kwargs['_visited_composed_classes'] = visited_composed_classes + (
            cls, )
    ...
    def __setitem__(self, name, value):
        ...
        if name not in self._var_name_to_model_instances:
            ...
            self.__dict__['_var_name_to_model_instances'][
                name] = self._composed_instances + [self]
        return None
...
COERCION_INDEX_BY_TYPE = {
    ...
    str: 11,
    file_type:
    12,  # 'file_type' is an alias for the built-in 'file' or 'io.IOBase' type.
}
...
def check_allowed_values(allowed_values, input_variable_path, input_values):
    ...
    if (isinstance(input_values, list)
            and not set(input_values).issubset(set(these_allowed_values))):
        invalid_values = ", ".join(
            map(str,
                set(input_values) - set(these_allowed_values))),
        raise ApiValueError(
            "Invalid values for `%s` [%s], must be a subset of [%s]" %
            (input_variable_path[0], invalid_values, ", ".join(
                map(str, these_allowed_values))))
    elif (isinstance(input_values, dict) and
          not set(input_values.keys()).issubset(set(these_allowed_values))):
        invalid_values = ", ".join(
            map(str,
                set(input_values.keys()) - set(these_allowed_values)))
        raise ApiValueError(
...
def is_json_validation_enabled(schema_keyword, configuration=None):
    ...
    return (configuration is None
            ...
            or schema_keyword
            not in configuration._disabled_client_side_validations)
...
def check_validations(validations,
    ...
    if (is_json_validation_enabled('multipleOf', configuration)
            and 'multiple_of' in current_validations
            and isinstance(input_values, (int, float))
            and not (float(input_values) /
                     current_validations['multiple_of']).is_integer()):
    ...
    if (is_json_validation_enabled('maxLength', configuration)
    ...
    if (is_json_validation_enabled('minLength', configuration)
    ...
    if (is_json_validation_enabled('maxItems', configuration)
    ...
    if (is_json_validation_enabled('minItems', configuration)
    ...
    if (is_json_validation_enabled('exclusiveMaximum', configuration)
    ...
    if (is_json_validation_enabled('maximum', configuration)
    ...
    if (is_json_validation_enabled('exclusiveMinimum', configuration)
    ...
    if (is_json_validation_enabled('minimum', configuration)
    ...
    if (is_json_validation_enabled('pattern', configuration)
            and 'regex' in current_validations
            and not re.search(current_validations['regex']['pattern'],
                              input_values,
                              flags=flags)):
        err_msg = r"Invalid value for `%s`, must match regular expression `%s`" % (
            input_variable_path[0], current_validations['regex']['pattern'])
        if flags != 0:
...
def remove_uncoercible(required_types_classes,
                       current_item,
...
def get_discriminated_classes(cls):
    ...
    for discr_cls in cls.discriminator[key].values():
        if hasattr(discr_cls,
                   'discriminator') and discr_cls.discriminator is not None:
...
def change_keys_js_to_python(input_dict, model_class):
    ...
    reversed_attr_map = {
        value: key
        for key, value in model_class.attribute_map.items()
    }
...
def get_type_error(var_value, path_to_item, valid_classes, key_type=False):
    error_msg = type_error_message(var_name=path_to_item[-1],
                                   var_value=var_value,
                                   valid_classes=valid_classes,
                                   key_type=key_type)
    return ApiTypeError(error_msg,
                        path_to_item=path_to_item,
                        valid_classes=valid_classes,
                        key_type=key_type)
    ...
    try:
        if klass in {datetime, date}:
            ...
            if klass == datetime:
                ...
                date_only = (parsed_datetime.hour == 0
        ...
        raise ApiValueError("{0}Failed to parse {1} as {2}".format(
            additional_message, repr(data), klass.__name__),
                            path_to_item=path_to_item) from ex
...
def attempt_convert_item(input_value,
                         valid_classes,
                         path_to_item,
                         configuration,
                         spec_property_naming,
    ...
    valid_classes_coercible = remove_uncoercible(valid_classes_ordered,
                                                 input_value,
                                                 spec_property_naming)
    if not valid_classes_coercible or key_type:
        ...
        if configuration is None or not configuration.discard_unknown_keys:
            raise get_type_error(input_value,
                                 path_to_item,
                                 valid_classes,
                                 key_type=key_type)
...
def is_valid_type(input_class_simple, valid_classes):
    ...
    if not valid_type and (issubclass(input_class_simple, OpenApiModel)
                           or input_class_simple is none_type):
        for valid_class in valid_classes:
            if input_class_simple is none_type and is_type_nullable(
                    valid_class):
                # Schema is oneOf/anyOf and the 'null' type is one of the allowed types.
                return True
            if not (issubclass(valid_class, OpenApiModel)
                    and valid_class.discriminator):
                continue
            ...
            valid_type = is_valid_type(input_class_simple,
                                       discriminator_classes)
            if valid_type:
                return True
    return valid_type
...
def validate_and_convert_types(input_value,
                               required_types_mixed,
                               path_to_item,
                               spec_property_naming,
                               _check_type,
    ...
    if len(valid_classes) > 1 and configuration:
        ...
        valid_classes_coercible = remove_uncoercible(valid_classes,
                                                     input_value,
                                                     spec_property_naming,
                                                     must_convert=False)
    ...
    if isinstance(input_value, list):
        ...
        for inner_key, inner_val in input_value.items():
            ...
            if get_simple_class(inner_key) != str:
                raise get_type_error(inner_key,
                                     inner_path,
                                     valid_classes,
                                     key_type=True)
...
def type_error_message(var_value=None,
                       var_name=None,
                       valid_classes=None,
                       key_type=None):
    """
    ...
    msg = ("Invalid type for variable '{0}'. Required {1} type {2} and "
    ...
    for allof_class in self._composed_schemas['allOf']:
        ...
        try:
            ...
            raise ApiValueError(
                ...
                "schema '%s'. Error=%s" %
                (allof_class.__name__, allof_class.__name__,
                 self.__class__.__name__, str(ex))) from ex
        ...
        try:
            if not single_value_input:
                if constant_kwargs.get('_spec_property_naming'):
                    ...
                    oneof_instance = oneof_class(**model_kwargs,
                                                 **constant_kwargs)
            else:
                if issubclass(oneof_class, ModelSimple):
                    if constant_kwargs.get('_spec_property_naming'):
                        ...
                        oneof_instance = oneof_class(model_arg,
                                                     **constant_kwargs)
                elif oneof_class in PRIMITIVE_TYPES:
                    oneof_instance = validate_and_convert_types(
                        model_arg, (oneof_class, ),
...
def validate_get_composed_info(constant_args, model_args, self):
    ...
    oneof_instance = get_oneof_instance(self.__class__, model_args,
                                        constant_args)
    ...
    for prop_name in model_args:
        if prop_name not in discarded_args:
            var_name_to_model_instances[prop_name] = [self
                                                      ] + composed_instances
